...
class SettingsWindow(QWidget):
    ...
    def __init__(self, config_manager):
        ...
        self.hotkey_input = QLineEdit(
            self.config_manager.get("hotkey", "<ctrl>+<alt>+w")
        )
        self.hotkey_input.setAccessibleName("Hotkey Input")
        self.hotkey_input.setAccessibleDescription(
            "Enter the keyboard shortcut to start and stop recording"
        )
        ...
        self.vad_input.setAccessibleDescription(
            "Adjust the sensitivity of voice detection"
        )
        ...
        self.model_input.setAccessibleDescription(
            "Select the size of the transcription model"
        )
        ...
        self.whisper_mode_input = QCheckBox(
            "Enable highly sensitive low-volume dictation"
        )
        self.whisper_mode_input.setChecked(
            self.config_manager.get("whisper_mode", False)
        )
        self.whisper_mode_input.setAccessibleName("Whisper Mode Checkbox")
        self.whisper_mode_input.setAccessibleDescription(
        ...
        logo_label = QLabel()
        logo_pixmap = QPixmap(get_asset_path("src/assets/branding/logo.png"))
        ...
        title_label = QLabel("<h2>WhisperAI</h2>")
        title_label.setAlignment(Qt.AlignCenter)
        about_layout.addWidget(title_label)

        version_label = QLabel("Version 1.1.0\nLocal Context-Aware Voice Dictation")
