...
from PySide6.QtCore import Qt, QPoint, QPropertyAnimation, QRect, QEasingCurve, QTimer, Signal, QSize
...
class FlowBubble(QWidget):
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.LeftButton:
            if self.click_pos is not None:
                dist = (event.globalPosition().toPoint() - self.click_pos).manhattanLength()
                if dist < 5:
                    self.clicked.emit()
            self.drag_pos = None
            self.click_pos = None
    ...
    def set_state(self, state: BubbleState):
        ...
        if state == BubbleState.IDLE:
            ...
            self.waveform.hide()
            self.spinner.show()
            self.icon_label.setStyleSheet("background-color: #F59E0B; border-radius: 15px;")
