...
def test_profiling_and_split_pass():
    ...
    # 5. Assertions
    assert peak_mem - baseline_mem < 1536, f"Memory footprint exceeded 1.5GB: {peak_mem - baseline_mem:.2f} MB (Peak: {peak_mem:.2f} MB, Baseline: {baseline_mem:.2f} MB)"
    ...
    assert len(token_speeds) >= 2, "Split-pass was not triggered (expected at least 2 LLM calls for mixed mode)"
