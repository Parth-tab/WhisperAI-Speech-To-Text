...
def test_detect_list_intent_with_number_ordinals():
    assert (
        detect_list_intent("number one finish the report number two send the email")
        is True
    )
...
def test_detect_list_mode_pure_list_from_start():
    ...
    assert (
        detect_list_mode("number one finish the report number two send the email")
        == "pure"
    )
...
def test_detect_list_mode_mixed_paragraph_then_list():
    ...
    assert (
        detect_list_mode(
            "The application is performing well. Number one it takes input."
        )
        == "mixed"
    )
...
def test_detect_list_mode_mixed_longer_prose():
    assert (
        detect_list_mode(
            "This project is going well. Now for action items: first update the docs."
        )
        == "mixed"
    )
...
def test_clean_text_pure_list_mode_enforces_newlines(mock_llama, mock_detect):
    ...
    mock_instance.return_value = {
        "choices": [
            {"text": "1. Finish the report. 2. Send the email. 3. Call the client."}
        ],
...
@patch(
    "src.llm.engine.LLMEngine._clean_text_mixed",
    return_value="The app works well.\n\n1. Takes input.\n2. Processes audio.",
)
@patch("src.llm.engine.Llama")
def test_clean_text_mixed_mode_delegates_to_split_pass(
    mock_llama, mock_mixed, mock_detect
):
    mock_instance = MagicMock()
    mock_llama.return_value = mock_instance
